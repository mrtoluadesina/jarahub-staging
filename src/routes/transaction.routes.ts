...
import { init, verify, getAllTransactions, getSingleTransaction } from '../controllers/transaction.controller';
...
router.get('/', async function(req: Request, res: Response) {
  const { statusCode, message, payload, error } = await getAllTransactions(req.query);
  return res.send({ statusCode, message, payload, error })
})

router.get('/:transactionId', async ( req: Request, res: Response ) => {
  const { statusCode, message, payload, error } = await getSingleTransaction(req.params.transactionId);
  return res.send({ statusCode, message, payload, error })
})
